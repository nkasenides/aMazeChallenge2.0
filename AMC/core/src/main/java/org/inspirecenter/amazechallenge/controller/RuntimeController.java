...
//                    game.getAudioEventListener().onGameEndAudioEvent(true); //TODO Remove, make for each player individually.
    ...
    private static boolean hasWall(final Grid grid, final MatrixPosition position, final Direction4 direction) {
        ...
        switch (direction) {
            case NORTH:
                return (shape & AMCTerrainGenerator.SHAPE_ONLY_UPPER_SIDE) != 0;
            case SOUTH:
                return (shape & AMCTerrainGenerator.SHAPE_ONLY_LOWER_SIDE) != 0;
            case WEST:
                return (shape & AMCTerrainGenerator.SHAPE_ONLY_LEFT_SIDE) != 0;
            case EAST:
                return (shape & AMCTerrainGenerator.SHAPE_ONLY_RIGHT_SIDE) != 0;
            default:
